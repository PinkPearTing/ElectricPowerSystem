...
class VectorFit3:
    ...
    def build_system_matrix(self, cindex, LAMBD, weight=1):
        ...
        """
        Dk = np.zeros((self.Ns, self.N), dtype=np.complex128)
    ...
    def find_pole(self, LAMBD, common_weight):
        ...
        if self.options['relax']:
            ...
            AA = np.zeros((self.Nc * (self.N + 1), self.N + 1), dtype=np.double)  # Set up AA to be used in Eq. (6), (A.8) in [1]
            bb = np.zeros((self.Nc * (self.N + 1), 1), dtype=np.double)           # Set up bb to be used in Eq. (6), (A.8) in [1]
            Escale = np.zeros((self.N + 1, 1), dtype=np.double)

            for n in range(self.Nc):
                A = np.zeros((self.Ns, (self.N + offs) + self.N + 1), dtype=np.complex128)
                ...
                A = np.vstack((np.real(A), np.imag(A)), dtype=np.double) # Stack the real and imaginary components as in Eq. (7) in [3]
        ...
        TOLlow = 1e-18
        TOLhigh = 1e18
        if self.options['relax'] == 0 or abs(x[-1]) < TOLlow or abs(x[-1]) > TOLhigh:
            AA = np.zeros((self.Nc * self.N, self.N))
            bb = np.zeros((self.Nc * self.N, 1))
            if self.options['relax'] == 0:
                Dnew = 1
            else:
                if x[-1] == 0:
                    Dnew = 1
            ...
            for n in range(self.Nc):
                A = np.zeros((self.Ns, (self.N + offs) + self.N), dtype=np.complex128)
                weig = self.weight[:, n] if not common_weight else self.weight[:, 0]

                for m in range(self.N + offs):  # 左块
                    A[:, m] = weig * Dk[:, m]
                inda = self.N + offs
                for m in range(self.N):  # 右块
                    A[:, inda + m] = -weig * Dk[:, m] * self.f[n, :].T  # conj()
                b = Dnew * weig * self.f[n, :].T  # conj()
                ...
                ind1 = self.N + offs
                ind2 = self.N + offs + self.N
                R22 = R[ind1:ind2, ind1:ind2]
                AA[n * self.N:(n + 1) * self.N, :] = R22.copy()
                bb[n * self.N:(n + 1)* self.N] = np.dot(Q[:, ind1 : ind2].T, b)

                # 缩放AA矩阵的列
                Escale = np.zeros((1, self.N))
        ...
        C = x[:-1].copy().astype(np.complex128)  # The C tilda residues become C sigma in Eq. (9) in [3]
        ...
        roetter = np.linalg.eigvals(np.real(ZER))   # The rest of Eq. (9) in [3] to find the zeros of sigma / new poles (called roetter)
        ...
        #         break
        #
        # # 如果存在实部为0的元素，则对后面的元素进行排序
        # if N1 < self.N:
        #     roetter[N1 + 1:self.N] = np.sort(roetter[N1 + 1:self.N])
        ...
        roetter = roetter - 2 * 1j * np.imag(roetter)   # Force the sorting to match that of MATLAB (negative imaginary
    ...
    def find_residue(self, roetter, common_weight):
        ...
        if common_weight == 1:
            ...
            SERD = np.zeros((self.Nc, 1), dtype=np.complex128)
            SERE = np.zeros((self.Nc, 1), dtype=np.complex128)
            ...
            SERD = np.zeros((self.Nc, 1), dtype=np.complex128)
            SERE = np.zeros((self.Nc, 1), dtype=np.complex128)
            C = np.zeros((self.Nc, self.N), dtype=np.complex128)
        ...
        fit = np.zeros((self.Nc, self.Ns), dtype=np.complex128)
        Dk = np.zeros((self.Ns, self.N), dtype=np.complex128)
        for m in range(0, self.N):
